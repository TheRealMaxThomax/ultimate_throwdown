...
**Often-used on `BallThrow`:** `ThrowForce`, `ThrowUpForce`, `ThrowReleaseDelaySeconds` (default `0.35` — ball velocity after anim wind-up), `ThrowAction` (`attack1`), `CancelChargeAction` (`attack2` — cancel charge without throwing), `IsChargingThrow`, `IsPendingThrowRelease`, `IsThrowPlantLocked` (owner charge + release-delay plant — no move/jump), `NetIsChargingThrow`, `NetThrowChargeLerp` (synced charge scrub for remotes), `ThrowDirectionSource` (optional; if unset, throw uses `PlayerController.EyeAngles`), `GetThrowChargeLerp()`, `TryGetThrowPreviewSnapshot()`, `CancelActiveThrowCharge()`, `CancelThrowAimingState()` (knockdown — clears charge + pending release)
...
| `PlayerFootstepAudio` | Owner: routes `PlayerController.FootstepMixer` to `Mixer.Master` on network spawn |
| `ThrowChargeCamera` | Owner-only throw-charge third-person pullback + mild FOV widen (scales with charge lerp) |
| `TackleImpactFeel` | Owner-only tackle connect juice — local camera hitstop, screen shake, attacker FOV/offset punch (`TackleImpactRelay` owner RPCs); optional per-impact overrides via `TackleImpactFeelOverrides` (Speed Blitz connect) |
| `TackleImpactFeelOverrides` | Per-impact tuning struct for `TackleImpactFeel.TriggerAsAttacker` / `TriggerAsVictim` |
| `SpeedBlitzDashCamera` | Owner-only FOV widen + third-person pullback during Speed Blitz dash; on Speedster prefab |
...
| `CombatFeelPredictDedupe` | Host `NetCombatFeelApplyId` + owner predict dedupe for combat feel RPCs (Tier A3 — model after `NetDodgeApplyId`); on player prefab |
...
**Often-used on `ThrowChargeCamera`:** `ExtraCameraDistanceAtFullCharge`, `ExtraCameraHeightAtFullCharge`, `ExtraFieldOfViewAtFullCharge`, `ReleaseCameraBlendDuration` — FOV in **`PlayerController.IEvents.PostCameraSetup`**; offset in `OnUpdate`; **`[Order(10002)]`** after **`BallThrow`**; transition FOV hold on release blend; idle must **not** stomp **`CameraOffset`**; skips when `PlayerTackle.IsRagdolled`, `IsStandUpCameraBlending`, `TackleImpactFeel.IsImpactFeelActive`, or active Speed Blitz

**Often-used on `TackleImpactFeel`:** `EnableHitstop`, `HitstopDurationSeconds` (default `0.055`), `ShakeForAttacker` / `ShakeForVictim` (default both on), `ShakeDurationSeconds`, `ShakePositionAmplitude`, `AttackerFovPunchDegrees`, `AttackerCameraOffsetPunchX` / `AttackerCameraOffsetPunchZ`, `AttackerPunchDurationSeconds`, `IsImpactFeelActive`, `IsHazardImpact`, `TriggerAsAttacker( overrides? )` / `TriggerAsVictim( overrides? )` (player tackle) / `TriggerAsHazardVictim()` (traffic — shake only, no hitstop, car camera path); FOV hitstop + attacker punch in **`PostCameraSetup`**; **blitz overrides** (`TackleImpactFeelOverrides`) — hitstop freezes world pose only (offset/FOV easing via **`SpeedBlitzDashCamera.BeginHitRecoveryBlend`**); no blitz attacker offset/FOV punch

**Often-used on `SpeedBlitzDashCamera`:** **Wind-up** — `ExtraFieldOfViewAtFullWindUp`, `ExtraCameraDistanceAtFullWindUp`, `ExtraCameraHeightAtFullWindUp`; **Dash** — `ExtraFieldOfViewDuringDash` (additive on dash peak), `ExtraCameraDistanceDuringDash`, `ExtraCameraHeightDuringDash`, `WindUpToDashBlendDurationSeconds` (wind-up peak → dash peak ease), `DashEndBlendDurationSeconds` (miss end + **hit recovery** to baseline); **`BeginHitRecoveryBlend()`** — owner call on enemy contact (from `SpeedsterSpeedBlitzUlt` before impact feel); FOV in **`PlayerController.IEvents.PostCameraSetup`**; offset in `OnUpdate`; `[Order(10012)]` right after ult phase; hit recovery keeps easing through blitz attacker hitstop (see `TackleImpactFeel` + overrides); skips when other `TackleImpactFeel.IsImpactFeelActive` or ragdolled

**Often-used on `BlitzConnectPoseFreeze`:** `IsBodyPoseFrozen`; reads `PlayerTackle.IsAwaitingSpeedBlitzRagdollLaunch` (victim) + `SpeedsterSpeedBlitzUlt.IsConnectPoseFrozen` (dasher); optional dasher snap via **`ConnectImpactChargeRunCycle`** on ult
...
| Name | Job |
|------|-----|
| `IPlayerUlt` | Per-ult hook — `MaxChargePoints` (raw points for 100%); implemented by each ult component |
| `PlayerUltCharge` | Host-authoritative 0–100% ult meter — passive regen (`Playing` only), goal/tackle bumps, rematch reset; resolves cap from equipped `IPlayerUlt`; `TrySpendFullChargeOnHost()` for ult commit |
| `SpeedsterSpeedBlitzUlt` | *(slice 2a)* Speedster **Speed Blitz** — hold/release `Ultimate` (2b) → wind-up → dash. Owner drives the dash through `PlayerController` (Rigidbody velocity → wall slide / step-up / ground-stick / running legs); host validates commit, times phases, runs **contact** hit-detection + knockdown, keeps dasher tackle-immune, blocks charge gain |
| `SpeedBlitzWindUpFeel` | Speed Blitz slice **2d** — wind-up electric VFX + wind-up/dash SFX + launch discharge burst; on Speedster prefab; driven by synced phase / connect freeze |
| `SpeedBlitzBodyGlow` | Speed Blitz dasher glow — ult blue tint + optional point light on avatar meshes; wind-up ramp, dash/connect peak, discharge at ragdoll launch; on Speedster prefab |
| `SpeedBlitzBodyGlowRenderSystem` | Scene pass — applies `SpeedBlitzBodyGlow` `SceneObject.ColorTint` at `Stage.Interpolation` after citizen LOD lock |
| `SpeedBlitzDashHitDetector` | Speed Blitz dash hit geometry — corridor filter, contact cylinder, line-of-sight trace; host + owner-predict share same methods; tunables read from `SpeedsterSpeedBlitzUlt` (Speedster prefab sibling) |
...
| `QuakeSlamAimPreview` | Owner-only preview — inner `oob_drop_ring` disc + mid/outer procedural annulus (`QuakeSlamPreviewAnnulusMesh`); `AnnulusSegmentCount` (default **64**); `RingSeamOverlap` preview-only seam tuck (default **2.5**) |
...
**Often-used on `SpeedsterSpeedBlitzUlt`:** **`MaxChargePoints`** (default **100** — tune on Speedster prefab), `IsActive`, `IsWindUp`, `IsDashing`, **`IsConnectPoseFrozen`**, **`SyncedPhase`**, `IsAiming`, `GetAimPreviewParams()`, `GetWindUpLerp()`, `BlocksBallPickup`, `CancelBlitzOnHost()`, `CancelAllInScene(scene)`; static **`PlayConnectImpactSoundAt`**, **`ResolveConnectImpactSoundVolume`**, **`TryConsumeHostConnectSoundDedupeForDasher`**; dash predict uses **`CombatFeelPredictDedupe.MarkOwnerPredictedAttackerFeel()`**; on hit calls **`SpeedBlitzDashCamera.BeginHitRecoveryBlend()`** before impact feel; tune `WindUpDurationSeconds` (default **2**), `DashRange`, `DashSpeed` (dash duration = range/speed), `HitHalfWidth` (outer corridor edge — matches preview side lines), **`MaxHitVerticalSeparation`** (default **56** — same vertical band as tackle; jumpers within band still connect), `DefaultTargetBodyRadius`, `DashSweepStepMultiplier`, `KnockdownLaunchSpeed`, `KnockdownLaunchArc`, **`KnockdownPreLaunchPauseSeconds`** (default **0.65** — blitz-only victim hang), **`ConnectImpactChargeRunCycle`** (default **-1** = freeze at contact; ≥ **0** = snap dasher `charge_run_cycle`), **`LaunchSound`** (drag-drop **`SoundEvent`** on Speedster prefab; code fallback `speed_blitz_launch.sound`), **`LaunchSoundVolume`**, **`DischargeVfxPrefab`** (fallback `vfx/speedblitzdischargevfx.prefab`), **`DischargeVfxLocalOffset`**, **`DischargeVfxCleanupSeconds`**, **`ImpactHitstopDurationSeconds`** / shake group under **Knockdown feel**; **2d wind-up feel:** **`WindUpVfxPrefab`** (fallback `vfx/speedblitzwindupvfx.prefab`), **`WindUpElectricSound`** / **`WindUpRiseSound`** / **`DashStartSound`** (fallbacks `speedblitz_electric`, `speedblitz_windup`, `speedblitz_dash`), **`WindUpElectricVolume`**, **`WindUpRemoteAttractorForceMultiplier`** (default **2.5** — observer pull), **`WindUpRemoteAttractorRadiusMultiplier`** (default **3**), **`WindUpRemoteLifetimeMultiplier`** (default **1.35**), **`MissVfxFadeSeconds`**
**Often-used on `SpeedBlitzConnectImpactRelay`:** **`ConnectImpactSoundA`** / **`ConnectImpactSoundB`** (drag-drop — host picks one at random per hit), **`ConnectImpactSoundVolume`**, owner predict dedupe via **`TryConsumeHostConnectSoundDedupe`**
...
**Often-used on `BallPassAssistState`:** `AssistWindowSeconds` (default **10**), `EnableAssistDebugLogs`, `NotifyThrowOnHost()`, `NotifyPickupOnHost()`, `TryGrantAssistChargeOnHost()`, `VoidOnEnemyTackleCarrierOnHost()`, `ResetOnHost()`, `Get(ball)`  
**Often-used on `PlayerUltCharge`:** `DefaultMaxChargePoints`, `PassivePointsPerSecond`, `GoalChargePoints`, `AssistChargePoints` (default **25**), `TackleChargePoints` (default **10**), `NetChargePercent`, `ChargePercent`, `IsFullyCharged`, `GrantGoalChargeOnHost()`, `GrantAssistChargeOnHost()`, `TryGrantTackleChargeOnHost()`, `TrySpendFullChargeOnHost()`, `ResyncFromEquippedUltOnHost()` (loadout swap — points carry over), `SetHostChargeGainBlocked(bool)` (no charge gain while an ult is active), `ResetAllPlayersInScene()`  
...
| `PracticeNpcPatrolPoseRelay` | Host fixed-tick broadcast of patrol runner pose to clients; on player prefab + patrol runner (`Code/Map/`) |
...
**Often-used on `JuggernautQuakeSlamUlt`:** **`MaxChargePoints`** (default **100**), `IsActive`, `IsWindUp`, `IsRingPhase`, **`SyncedPhase`**, **`SyncedRingPulseIndex`**, `IsAiming`, `GetAimPreviewParams()`, `GetWindUpLerp()`, `GetSlamOriginWorld()`, `BlocksBallPickup`, `CancelQuakeSlamOnHost()`, `CancelAllInScene(scene)`; tune `WindUpDurationSeconds` (default **1.5**), `InnerRadius` / `MidRadius` / `OuterRadius` (70 / 135 / 200), `RingPhaseDelaySeconds` (default **0.5**), `MaxHitVerticalSeparation`, `KnockdownLaunchSpeed`, `KnockdownLaunchArc`, `OutwardKnockFraction`, `InnerRingPowerMultiplier` / `MidRingPowerMultiplier` / `OuterRingPowerMultiplier`
**Loadout catalog ids:** class `speedster` / `juggernaut` / `sniper`; ult `speed_blitz` / `quake_slam`; passive `default` / `tackle_ramp`
