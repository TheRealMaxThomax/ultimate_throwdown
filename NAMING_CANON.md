...
**Often-used on `SpeedBlitzAimPreview`:** `CorridorTint`, `CorridorAlpha`, `CorridorHeight`, `CorridorLift`, `SegmentSpacing`, `MaxSegments`, `MarkerTint`, `MarkerAlpha`, `MarkerHeight`, `MarkerLift`, `MarkerMaterialPath`, `MarkerModelBaseSize`, `StepUpHeightFallback` (uses `MoveModeWalk.StepUpHeight` when present)  
