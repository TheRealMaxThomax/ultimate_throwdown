...
**Often-used on `SpeedsterSpeedBlitzUlt`:** `IsActive`, `IsWindUp`, `IsDashing`, **`IsConnectPoseFrozen`**, `IsAiming`, `GetAimPreviewParams()`, `BlocksBallPickup`, `CancelBlitzOnHost()`, `CancelAllInScene(scene)`; dash predict uses **`CombatFeelPredictDedupe.MarkOwnerPredictedAttackerFeel()`**; on hit calls **`SpeedBlitzDashCamera.BeginHitRecoveryBlend()`** before impact feel; tune `WindUpDurationSeconds` (default **2**), `DashRange`, `DashSpeed` (dash duration = range/speed), `HitHalfWidth` (outer corridor edge — matches preview side lines), `DefaultTargetBodyRadius`, `DashSweepStepMultiplier`, `KnockdownLaunchSpeed`, `KnockdownLaunchArc`, **`KnockdownPreLaunchPauseSeconds`** (default **0.65** — blitz-only victim hang), **`ConnectImpactChargeRunCycle`** (default **-1** = freeze at contact; ≥ **0** = snap dasher `charge_run_cycle`), **`LaunchSound`** (drag-drop **`SoundEvent`** on Speedster prefab; code fallback `speed_blitz_launch.sound`), **`LaunchSoundVolume`**, **`ConnectImpactSoundA`** / **`ConnectImpactSoundB`** (drag-drop — host picks one at random per hit), **`ConnectImpactSoundVolume`**, **`ImpactHitstopDurationSeconds`** / shake group under **Knockdown feel**; auto-adds **`SpeedBlitzDashCamera`**
