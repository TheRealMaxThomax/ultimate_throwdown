...
**Often-used on `SpeedsterSpeedBlitzUlt`:** `IsActive`, `IsWindUp`, `IsDashing`, `IsAiming`, `GetAimPreviewParams()`, `BlocksBallPickup`, `CancelBlitzOnHost()`, `CancelAllInScene(scene)`; tune `WindUpDurationSeconds`, `DashRange`, `DashSpeed` (dash duration = range/speed), `HitHalfWidth` (outer corridor edge — matches preview side lines), `DefaultTargetBodyRadius`, `DashSweepStepMultiplier`, `KnockdownLaunchSpeed`, `KnockdownLaunchArc`  
