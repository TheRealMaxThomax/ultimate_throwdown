...
| `PlayerBallHoldAnim` | Built-in citizen `holditem` RH hold + throw on release (`holdtype`, `holdtype_pose_hand`, `holdtype_attack`, `b_attack`); charge wind-up via forked animgraph masked layer (`throw_charge` / `throw_charge_weight` params — body stays alive) with direct-playback fallback; pairs with `BallThrow.ThrowReleaseDelaySeconds` for ball detach timing |
...
**Often-used on `PlayerBallHoldAnim`:** `BodyRenderer`, `IdleHoldPoseHand` (default `0.1`), `ThrowAttackStrong` (default `0` = medium throw; `1` = strong), `ThrowPoseHoldSeconds` (default `0.9`), `ThrowPlaybackRate` (default `0.7` during throw window), **`UseAnimGraphChargePose`** (default `true` — preferred charge route: masked layer in forked citizen graph, body keeps locomotion/look-at), **`CustomAnimGraphPath`** (default `animation/utd_citizen_human_m.vanmgrph`), **`ChargeCycleParamName`** (default `throw_charge` — graph float param scrubbing the wind-up Single Frame node), **`ChargeWeightParamName`** (default `throw_charge_weight` — graph float param blending the masked layer in/out), **`ChargeWeightBlendInSeconds`** (default `0.12`), **`ChargeWeightBlendOutSeconds`** (default `0.15`), `EnableBuiltInChargeWindup` (default `false`), `IdleHoldTypePose`, `ChargeHoldTypePoseStart`, `ChargeHoldTypePoseEnd`, `ChargeHoldPoseHandStart`, `ChargeHoldPoseHandEnd`, `EnableCustomChargePose` (default `true` — direct-playback fallback, full-body frozen pose), `CustomBodyModelPath` (default `animation/utd_citizen_human_throw.vmdl`), `EnsureCustomBodyModel()` (re-applies Body model **and** custom animgraph after cosmetics), `ChargeWindupSequenceName` (default `throw_windup`), `UseDeltaChargeSequences` (default `false` — deltas break on direct-playback scrub), `ChargeSequenceDeltaSuffix` (default `_delta`), `HoldReadySequenceName`, `ChargeMinSequenceName`, `ChargeMaxSequenceName`, `ChargeHoldReadyPhaseEnd` (default `0.2`), `ChargeMinPhaseEnd` (default `0.5`), `NotifyThrowReleased()`

**Custom citizen animation assets:** `Assets/Animation/utd_citizen_human_throw.vmdl` (Base Model = `citizen_human_male`; add all custom sequences here) and `Assets/Animation/utd_citizen_human_m.vanmgrph` (fork of Facepunch `citizen_human_m.vanmgrph` + masked charge layer driven by `throw_charge` / `throw_charge_weight`). Workflow: [`Assets/Animation/CITIZEN_ANIMATION_WORKFLOW.md`](Assets/Animation/CITIZEN_ANIMATION_WORKFLOW.md)
