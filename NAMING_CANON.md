...
**Often-used on `SpeedsterSpeedBlitzUlt`:** **`MaxChargePoints`** (default **100** — tune on Speedster prefab), `IsActive`, `IsWindUp`, `IsDashing`, **`IsConnectPoseFrozen`**, **`SyncedPhase`**, `IsAiming`, `GetAimPreviewParams()`, `GetWindUpLerp()`, `BlocksBallPickup`, `CancelBlitzOnHost()`, `CancelAllInScene(scene)`; dash predict uses **`CombatFeelPredictDedupe.MarkOwnerPredictedAttackerFeel()`**; on hit calls **`SpeedBlitzDashCamera.BeginHitRecoveryBlend()`** before impact feel; tune `WindUpDurationSeconds` (default **2**), `DashRange`, `DashSpeed` (dash duration = range/speed), `HitHalfWidth` (outer corridor edge — matches preview side lines), **`MaxHitVerticalSeparation`** (default **56** — same vertical band as tackle; jumpers within band still connect), `DefaultTargetBodyRadius`, `DashSweepStepMultiplier`, `KnockdownLaunchSpeed`, `KnockdownLaunchArc`, **`KnockdownPreLaunchPauseSeconds`** (default **0.65** — blitz-only victim hang), **`ConnectImpactChargeRunCycle`** (default **-1** = freeze at contact; ≥ **0** = snap dasher `charge_run_cycle`), **`LaunchSound`** (drag-drop **`SoundEvent`** on Speedster prefab; code fallback `speed_blitz_launch.sound`), **`LaunchSoundVolume`**, **`ConnectImpactSoundA`** / **`ConnectImpactSoundB`** (drag-drop — host picks one at random per hit), **`ConnectImpactSoundVolume`**, **`DischargeVfxPrefab`** (fallback `vfx/speedblitzdischargevfx.prefab`), **`DischargeVfxLocalOffset`**, **`DischargeVfxCleanupSeconds`**, **`ImpactHitstopDurationSeconds`** / shake group under **Knockdown feel**; **2d wind-up feel:** **`WindUpVfxPrefab`** (fallback `vfx/speedblitzwindupvfx.prefab`), **`WindUpElectricSound`** / **`WindUpRiseSound`** / **`DashStartSound`** (fallbacks `speedblitz_electric`, `speedblitz_windup`, `speedblitz_dash`), **`WindUpElectricVolume`**, **`WindUpRemoteAttractorForceMultiplier`** (default **2.5** — observer pull), **`WindUpRemoteAttractorRadiusMultiplier`** (default **3**), **`WindUpRemoteLifetimeMultiplier`** (default **1.35**), **`MissVfxFadeSeconds`**; auto-adds **`SpeedBlitzDashCamera`**, **`SpeedBlitzWindUpFeel`**, **`SpeedBlitzBodyGlow`**
