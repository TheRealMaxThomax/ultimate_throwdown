...
**Often-used on `SpeedsterSpeedBlitzUlt`:** `IsActive`, `IsWindUp`, `IsDashing`, `IsAiming`, `GetAimPreviewParams()`, `BlocksBallPickup`, `CancelBlitzOnHost()`, `CancelAllInScene(scene)`; dash predict uses **`CombatFeelPredictDedupe.MarkOwnerPredictedAttackerFeel()`**; on hit calls **`SpeedBlitzDashCamera.BeginHitRecoveryBlend()`** before impact feel; tune `WindUpDurationSeconds` (default **2**), `DashRange`, `DashSpeed` (dash duration = range/speed), `HitHalfWidth` (outer corridor edge — matches preview side lines), `DefaultTargetBodyRadius`, `DashSweepStepMultiplier`, `KnockdownLaunchSpeed`, `KnockdownLaunchArc`, **`KnockdownPreLaunchPauseSeconds`** (default **0.65** — blitz-only victim hang), **`ImpactHitstopDurationSeconds`** / shake group under **Knockdown feel**; auto-adds **`SpeedBlitzDashCamera`**
