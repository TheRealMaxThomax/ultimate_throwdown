...
- **Ball carrier hold/throw anim (v1):** **`PlayerBallHoldAnim`** — **`holditem`** + **RH** while holding; on release **`b_attack`**. Charge = masked **`throw_windup`** on forked **`utd_citizen_human_m.vanmgrph`** (`throw_charge` / `throw_charge_weight`). **RMB cancel** → **`NotifyThrowChargeCancelled`** (weight + cycle ease — `ChargeCancelBlendOutSeconds`). **`BallThrow.IsThrowPlantLocked`** — planted feet during charge + release delay (**`CatchUpSpeedBoost`** + jump off). **`ThrowReleaseDelaySeconds`** delays ball velocity. Auto-added on network spawn.
