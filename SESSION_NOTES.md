...
## CRITICAL — MP chunk limit (`AssembleChunk` / 1024) — DO NOT REGRESS (2026-05-11)
...
- **Critical `.sbproj` — resources vs MP:** **`"Resources": null`** is required for stable **client join** (see **CRITICAL — MP chunk limit** at top). **`"Resources": "*"`** has caused **`Chunk total … exceeds 1024 limit`** when the engine tries to sync too much. If you need map assets locally, rely on **map compile / baked refs** and **targeted** packaging — not blanket `*` without multi-instance testing. **`MapList`** includes **`testing_map`** and **`facepunch.flatgrass`** (ids are logical strings, not filenames).
...
- **2026-05-10 (Hammer / arena):** Added **`Assets/Maps/testing_map.vmap`** pipeline: **`Code/Map/StartupMapBootstrap.cs`** mounts **`MapInstance`** with **`MapName` `testing_map`** (host); **`Metadata.MapList`** updated. *Historical:* briefly used **`"Resources": "*"`** to ship **`maps/testing_map.vpk`** — **reverted to `null` (2026-05-11)** because **`"*"`** caused **`Connection.AssembleChunk` / “Chunk total exceeds 1024”** on client join; see **CRITICAL** at top of SESSION_NOTES. Concrete/`AmbientCG` workflow noted; interior lighting time sink → **roof removed**, **`light_environment`** sun; **`full`** compile when brightness bake stuck vs **`entities-only`**; **viewport black-until-Stop→Play** after compile. Optional scene **`MapInstance`** + remove duplicate grass floor when aligned.
