...
**Status:** **Slices 1–4 shipped and playtested (2-window MP).** Slices 5–6 not started.  
...
`Read SESSION_NOTES.md and MATCH_FLOW_PLAN.md. Continue from slice 5 (match HUD). Do not edit .scene files unless I ask.`
...
- `Code/Match/MatchDirector.cs` — host phase machine; synced round wins, match timer, phase timer, OT flag; `RegisterGoal()`; `IsGameplayInputAllowed`; `PushMatchPhaseToPlayers()`; `PerformRoundReset()`; `HostRequestRematch()` stub.
...
- `MatchDirector.PushMatchPhaseToPlayers()` — copies `NetPhase` onto every networked `PlayerTeam` (see networking note below).
...
- Join mid-intermission — `SyncMatchPhaseToPlayer()` + pickup block for remaining `NetPhaseTimeRemaining`.

**Networking (important):**
- `MatchDirector` lives on **local** Main Camera per machine — its `[Sync]` fields do **not** drive client freeze/teleport by themselves.
- **Authoritative for clients:** `PlayerTeam` on each **network-spawned player** (`NetMatchPhase`, reset sequence/position).
- Host still owns phase transitions and reset orchestration; clients react to synced player fields.
...
- [ ] **`GoalBannerHud`** — e.g. `"TEAM A SCORED!"` during `GoalCelebration` (use `NetLastGoalScoringTeamId` + display names; phase from local `PlayerTeam.NetMatchPhase` or host `MatchDirector` on host machine).
...
```text
Playing
  → GoalCelebration (5s)   // move freely
...
| `MatchDirector` | Phase machine + `BallSpawn` ref |
...
- Clients: read match phase + reset from **`PlayerTeam`** on their pawn; HUD reads synced director fields on host / `PlayerTeam` on clients.
- Round wins / timer: still on `MatchDirector` `[Sync]` (fine for HUD on host; clients may read via future HUD or replicated score fields).
...
| Component | Status |
|-----------|--------|
| `MapMatchConfig`, `MatchTeamIds`, `PlayerTeam` | Built (phase + reset sync on `PlayerTeam`) |
